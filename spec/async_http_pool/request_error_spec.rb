...
RSpec.describe AsyncHttpPool::RequestError do
  describe ".from_exception" do
    ...
    context "with ResponseTooLargeError" do
      it "classifies as :response_too_large" do
        exception = AsyncHttpPool::ResponseTooLargeError.new("Response too large")
        error = described_class.from_exception(exception, request_id: request_id, duration: 1.0, url: url,
          http_method: :get)

        expect(error.error_class).to eq(AsyncHttpPool::ResponseTooLargeError)
    ...
    context "with callback_args" do
      it "includes callback_args in the error" do
        ...
        expect(error.callback_args).to be_a(AsyncHttpPool::CallbackArgs)
  ...
  describe ".error_type" do
    it "classifies errors correctly" do
      ...
      expect(described_class.error_type(AsyncHttpPool::ResponseTooLargeError.new)).to eq(:response_too_large)
