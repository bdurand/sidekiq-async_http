...
RSpec.describe AsyncHttpPool::ResponseReader do
  let(:config) { AsyncHttpPool::Configuration.new }
  let(:processor) { instance_double(AsyncHttpPool::Processor, config: config, stopping?: false, stopped?: false) }
  ...
  describe "#read_body" do
    ...
    context "when content-length exceeds max_response_size" do
      ...
      it "raises ResponseTooLargeError" do
        ...
        }.to raise_error(AsyncHttpPool::ResponseTooLargeError, /10000001 bytes.*exceeds maximum/)
      end
    end
    ...
    context "when body exceeds max_response_size during reading" do
      ...
      it "raises ResponseTooLargeError" do
        expect {
          response_reader.read_body(async_response, headers_hash)
        }.to raise_error(AsyncHttpPool::ResponseTooLargeError, /exceeded maximum allowed size/)
      end
      ...
      it "closes the body on error" do
        ...
        expect {
          response_reader.read_body(async_response, headers_hash)
        }.to raise_error(AsyncHttpPool::ResponseTooLargeError)
      end
    end
