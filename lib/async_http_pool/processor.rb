  ...
  class Processor
    ...
    def start
      ...
      @reactor_thread = Thread.new do
        ...
        @config.logger&.error("[AsyncHttpPool] Processor error: #{e.message}\n#{e.backtrace.join("\n")}")

        raise if AsyncHttpPool.testing?
      ensure
    ...
    def drain
      ...
      @config.logger&.info("[AsyncHttpPool] Processor draining (no longer accepting new requests)")
    end
    ...
    def run_reactor
      Async do |task|
        ...
        @config.logger&.info("[AsyncHttpPool] Processor started")
        ...
        loop do
          ...
          task.async do
            ...
            @config.logger&.error("[AsyncHttpPool] Error processing request: #{e.inspect}\n#{e.backtrace.join("\n")}")

            warn(e.inspect, e.backtrace) if AsyncHttpPool.testing?
          end
        end

        @config.logger&.info("[AsyncHttpPool] Processor stopped")
      rescue Async::Stop
        @config.logger&.info("[AsyncHttpPool] Reactor received stop signal")
      rescue => e
        @config.logger&.error("[AsyncHttpPool] Reactor loop error: #{e.inspect}\n#{e.backtrace.join("\n")}")
      end
    end
    ...
    def process_request(task)
      ...
      begin
        ...
        @testing_callback&.call(task) if AsyncHttpPool.testing?
      end
    end
    ...
    def handle_completion(task, response)
      if stopped?
        @config.logger&.warn("[AsyncHttpPool] Request #{task.id} succeeded after processor was stopped")
        return
      end
      ...
      @config.logger&.debug(
        "[AsyncHttpPool] Request #{task.id} succeeded with status #{response.status}, " \
    ...
    def handle_redirect(task, response_data)
      ...
      @config.logger&.debug("[AsyncHttpPool] Request #{task.id} redirected (#{status}) to #{redirect_url}")
    end
    ...
    def handle_error(task, exception)
      if stopped?
        @config.logger&.warn("[AsyncHttpPool] Request #{task.id} failed after processor was stopped")
        return
      end
      ...
      @config.logger&.warn(
        "[AsyncHttpPool] Request #{task.id} failed with #{exception.class.name}: #{exception.message}, " \
        "enqueued callback #{task.callback}\n#{exception.backtrace&.join("\n")}"
      )
    rescue => e
      @config.logger&.error(
        "[AsyncHttpPool] Failed to enqueue error worker for request #{task.id}: #{e.class} - #{e.message}"
      )
      raise if AsyncHttpPool.testing?
    end
    ...
    def notify_observers(&block)
      @observers.each do |observer|
        ...
        @config.logger&.error(
          "[AsyncHttpPool] Observer #{observer.class.name} error: #{e.class} - #{e.message}"
        )
        raise e if AsyncHttpPool.testing?
      end
    end
    ...
    def reenqueue_pending_requests
      ...
      tasks_to_reenqueue.each do |task|
        ...
        @config.logger&.info(
          "[AsyncHttpPool] Retrying incomplete request #{task.id}"
        )
      rescue => e
        @config.logger&.error(
          "[AsyncHttpPool] Failed to re-enqueue request #{task.id}: #{e.class} - #{e.message}"
        )
